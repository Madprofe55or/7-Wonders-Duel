	...
	while (p_game->window.pollEvent(event))
	{
		...
		case sf::Event::MouseMoved:
		{
			...
			for (vector < sf::FloatRect*>::iterator it = mCardRects.begin(); it != mCardRects.end(); ++it)
			{
				...
				{
					mouseoverCard.setTexture(p_game->textureManager.getRef(p_game->world.currentPlayer->playerCity[mouseoverVectorCount]->getName()));
					mouseover = true;
					//break;
				}
...
ViewingCityState::ViewingCityState(Game * game, GamePlayingState * gameplayingstate)
{
	p_game = game;
	...
	if (!p_game->world.currentPlayer->playerCity.empty()) // need to first check that there are actually cards in the player's city
	{
		for (vector<Card*>::iterator it = p_game->world.currentPlayer->playerCity.begin();
			it != p_game->world.currentPlayer->playerCity.end(); ++it)
		{
